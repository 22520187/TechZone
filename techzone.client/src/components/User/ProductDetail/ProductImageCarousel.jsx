      ...
      <div className="relative w-full h-[300px] sm:h-[400px] md:h-[300px] overflow-hidden bg-muted/20">
        ...
        >
          {images && images.length > 0 ? images.map((image, index) => (
            <div
              key={index}
              className={`absolute inset-0 transition-opacity duration-500 flex justify-center ${index === currentIndex ? "opacity-100 z-10" : "opacity-0 z-0"
                }`}
            >
              {/* Loading Placeholder */}
              {!imagesLoaded[index] && (
        ...
        <div className="flex justify-center mt-4 space-x-2 px-4">
          {images && images.length > 0 ? images.map((image, index) => (
            <button
              ...
              className={`relative cursor-pointer w-16 h-16 md:w-20 md:h-20 border-2 rounded-md overflow-hidden transition-all duration-200 ${index === currentIndex
                  ? "border-primary"
