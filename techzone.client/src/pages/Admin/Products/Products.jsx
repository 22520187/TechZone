...
export default function Products() {
    ...
    const [colors, setColors] = useState([
        { color: "", colorCode: "#000000", stockQuantity: 0 },
    ]);
    ...
    const showEditModal = (product) => {
        ...
        const productData = productItems.find(
    ...
    const handleSubmit = async (values) => {
        try {
            ...
            if (
                colors.length === 0 ||
                colors.some((c) => !c.color || c.stockQuantity < 0)
            ) {
                message.error(
                    "Please provide valid colors and stock quantities"
                );
                setSubmitting(false);
                return;
            }
            ...
            if (selectedFile) {
                ...
                if (uploadedImageUrl) {
                    ...
                    updatedImageUrls = [
                        uploadedImageUrl,
                        ...updatedImageUrls.filter((url) => url !== ""),
                    ];
                }
            }
            ...
            const productData = {
                ...
                longDescription:
                    values.longDescription || values.description || "",
                price: values.price,
                ...
                >
                    {filterProductsData.length === 0 ? (
                        <div className="col-span-full text-center py-10 text-gray-500">
                            No products found. Try a different search or add a
                            new product.
                        </div>
                    ) : (
