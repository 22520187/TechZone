...
{
    public class SQLPromotionRepository : TechZoneRepository<Promotion>, IPromotionRepository
    {
        public SQLPromotionRepository(TechZoneDbContext dbContext) : base(dbContext)
        {
        }
        ...
        public async Task LinkProductsToPromotionAsync(Promotion promotion, ICollection<string> productIds)
        {
            ...
            {
                var productsToRemove = existingProducts.Where(p => !newProducts.Contains(p)).ToList();
