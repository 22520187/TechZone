using TechZone.Server.Controllers;
...
public interface IChatbotKnowledgeService
{
    ...
    /// </summary>
    Task<ChatbotAnswer?> TryAnswerFromDatabaseAsync(string message, int? userId);
    ...
    /// <summary>
    /// Trích xuất danh sách sản phẩm từ database context để hiển thị ở frontend.
    /// </summary>
    Task<List<ProductCardDto>?> ExtractProductsFromContextAsync(string message, int? userId, int maxProducts = 5);
}
